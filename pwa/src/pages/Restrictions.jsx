...
const PRESET_CATEGORIES = [
  { id: 'adult', label: 'Adult Content', domains: [] },
...
export default function Restrictions() {
  ...
  const [restrictions, setRestrictions] = useState({ mode: 'blocklist', domains: [] });
  ...
  function removeDomain(domain) {
    const updated = { ...restrictions, domains: restrictions.domains.filter((d) => d !== domain) };
    setRestrictions(updated);
    save(updated);
  }
    ...
    <div>
      <div style={styles.topBar}>
        <div>
          ...
          <p style={styles.subtitle}>Block or allow specific websites. Changes apply within 60 seconds on the child's Mac.</p>
        </div>
        ...
        <>
          <div style={styles.card}>
            ...
            <div style={styles.modeRow}>
              <ModeCard
                active={restrictions.mode === 'blocklist'}
                icon="🚫"
                title="Blocklist"
                desc="All sites allowed except those on the list below."
                onClick={() => setMode('blocklist')}
              />
              <ModeCard
                active={restrictions.mode === 'allowlist'}
                icon="✅"
                title="Allowlist"
                desc="All sites blocked except those on the list below."
          ...
          <div style={styles.card}>
            ...
            <div style={styles.presetRow}>
              {PRESET_CATEGORIES.filter((p) => p.domains.length > 0).map((preset) => (
          ...
          <div style={styles.card}>
            ...
            {restrictions.domains.length === 0 ? (
              ...
              <div style={styles.domainList}>
                {restrictions.domains.map((domain) => (
                  <div key={domain} style={styles.domainRow}>
                    <span style={styles.domainIcon}>{restrictions.mode === 'blocklist' ? '🚫' : '✅'}</span>
