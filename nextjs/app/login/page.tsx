...
import { firebaseLogin, firebaseRegister } from '@/lib/firebase';
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <div style={styles.tabs}>
          <button onClick={() => setMode('login')} style={{ ...styles.tab, ...(mode === 'login' ? styles.tabActive : {}) }}>
            Sign in
          </button>
          <button onClick={() => setMode('register')} style={{ ...styles.tab, ...(mode === 'register' ? styles.tabActive : {}) }}>
            Create account
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <label style={styles.label}>
            Password
            <input style={styles.input} type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={mode === 'register' ? 'At least 6 characters' : ''} required />
          </label>
...
const styles: Record<string, React.CSSProperties> = {
  container: { minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #1e1b4b 0%, #312e81 100%)' },
  ...
  tabs: { display: 'flex', background: '#f3f4f6', borderRadius: 8, padding: 4, marginBottom: 24 },
  tab: { flex: 1, padding: '8px 0', border: 'none', background: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 14, color: '#6b7280' },
  ...
  btn: { padding: '12px', background: '#4f46e5', color: '#fff', border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: 'pointer', marginTop: 4 },
};
