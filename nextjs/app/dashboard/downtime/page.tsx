...
const DEFAULT_DOWNTIME = { enabled: false, start: '22:00', end: '07:00', allowed_apps: [] };

export default function DowntimePage() {
  const { selected } = useDevice();
  const [downtime, setDowntime] = useState<any>(DEFAULT_DOWNTIME);
  ...
  function update(field: string, value: any) {
    ...
    setDowntime(updated); save(updated);
  }

  function addAllowedApp() {
    if (!newApp.trim()) return;
    const updated = { ...downtime, allowed_apps: [...downtime.allowed_apps, newApp.trim()] };
    setDowntime(updated); save(updated); setNewApp('');
  }

  function removeAllowedApp(app: string) {
    const updated = { ...downtime, allowed_apps: downtime.allowed_apps.filter((a: string) => a !== app) };
    setDowntime(updated); save(updated);
  }
        ...
        <>
          <div style={styles.card}>
            <div style={styles.row}>
              ...
              <label style={styles.toggle}>
                <input type="checkbox" checked={downtime.enabled} onChange={(e) => update('enabled', e.target.checked)} style={{ display: 'none' }} />
            ...
            {downtime.enabled && (
              <div style={styles.timeRow}>
                <label style={styles.timeLabel}>
                  Start time
                  <input type="time" value={downtime.start} onChange={(e) => update('start', e.target.value)} style={styles.timeInput} />
                </label>
                <span style={{ fontSize: 20, color: '#9ca3af' }}>to</span>
                <label style={styles.timeLabel}>
                  End time
                  <input type="time" value={downtime.end} onChange={(e) => update('end', e.target.value)} style={styles.timeInput} />
                </label>
              </div>
            )}
          </div>
          ...
          <div style={styles.card}>
            ...
            <div style={styles.addRow}>
              <input style={styles.input} placeholder="App name (e.g. Messages)" value={newApp}
            ...
            <div style={styles.tags}>
              {downtime.allowed_apps.length === 0
                ...
                : downtime.allowed_apps.map((app: string) => (
                  <span key={app} style={styles.tag}>
                    {app}
                    <button onClick={() => removeAllowedApp(app)} style={styles.tagRemove}>x</button>
                  </span>
                ))
              }
            </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  toggleTrack: { display: 'block', width: 52, height: 28, borderRadius: 99, position: 'relative' },
  toggleThumb: { position: 'absolute', top: 2, width: 24, height: 24, borderRadius: '50%', background: '#fff', boxShadow: '0 1px 3px rgba(0,0,0,0.2)' },
