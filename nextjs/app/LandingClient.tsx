    ...
    return (
      <div style={s.platformPanel}>
        ...
        <ol style={s.stepsList}>
          ...
          <li>Right-click <code style={s.inlineCode}>oversight-windows.bat</code> and "Run as administrator"</li>
  ...
  return (
    <div style={s.platformPanel}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 8 }}>
        <a href="/downloads/oversight-mac-apple-silicon.zip" download="oversight-mac-apple-silicon.zip" style={s.bigDownloadBtn}>
          <Icon name="download" size={18} color="#fff" />
          <span>Apple Silicon (M1/M2/M3)</span>
        </a>
        <a href="/downloads/oversight-mac-intel.zip" download="oversight-mac-intel.zip" style={s.bigDownloadBtn}>
          <Icon name="download" size={18} color="#fff" />
          <span>Intel Mac</span>
        </a>
      </div>
      <ol style={s.stepsList}>
        <li>Open your Downloads folder and double-click the downloaded <code style={s.inlineCode}>.zip</code> to unzip it</li>
...
function ParentPanel() {
  ...
  return (
    <div style={s.platformPanel}>
      ...
      <button onClick={() => router.push('/login')} style={{ ...s.bigDownloadBtnBtn, marginTop: 24 }}>
        Open Dashboard
      </button>
    </div>
  );
...
function RoleModal({ onClose }: { onClose: () => void }) {
  const [view, setView] = useState<'choose' | 'parent' | 'child'>('choose');
  return (
    ...
    <div style={s.page}>
      <header style={s.nav}>
        ...
        <div style={s.navLinks}>
          <button onClick={() => router.push('/login')} style={s.navBtn}>Sign In</button>
          <button onClick={() => router.push('/login')} style={s.navBtnOutline}>Create Account</button>
        </div>
      </header>

      <section style={s.hero}>
        <div style={{ marginBottom: 16 }}><Icon name="eye" size={56} color="#818cf8" /></div>
        ...
        <p style={s.heroSub}>Monitor usage, block sites, set time limits, and view live screenshots — all from one dashboard. Works on Mac, Windows, Linux, iOS &amp; Android.</p>
      ...
      <footer style={s.footer}>
        ...
        <button onClick={() => router.push('/login')} style={s.footerLink}>Create Account</button>
        <span style={{ display: 'block', marginTop: 12, color: '#4b5563', fontSize: 12 }}>
          © 2025 Oversight by BigHappySmiley. Built for families.
        </span>
      </footer>
    </div>
  );
...
const s: Record<string, React.CSSProperties> = {
  ...
  modalWrapper: { marginTop: 32, textAlign: 'left' },
  ...
  roleCard: { flex: '1 1 200px', background: '#1e1e3a', border: '2px solid #312e81', borderRadius: 12, padding: '24px', cursor: 'pointer', textAlign: 'left', color: '#f1f5f9' },
